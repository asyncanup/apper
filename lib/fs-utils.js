        ...
        function optimize() {
            return function () {
                var newElement = $("<script>"),
                    requireElement = $("<script>"),
                    relativeFilePath = $(this).attr("data-main");
                
                $(this).after(requireElement);
                requireElement.attr("src", $(this).attr("data-main") + ".js");
                ...
                app.log("Optimizing: " + relativeFilePath);
                
                var requireConfig = {};
                try {
                    requireConfig = require(path.join(baseUrl, "require-config"));
                } catch (e) {
                    app.log("No require-config found");
                    app.log(e.message);
                }
                ...
                requirejs.optimize(
                    ...
                    function () {
                        fs.readFile(outFile, function (err, builtContents) {
                            if (err) {
                                app.log("Couldn't read created build file: " + outFile);
                                partDone();
                                return;
                            }
                            ...
                            fs.unlink(outFile, function (err) {
                                if (err) {
                                    app.log("Could not delete built file: " + outFile);
                                    return;
                                }
                    ...
                    app.log("Could not optimize: " + relativeFilePath);
                    app.log(err.message);
                    partDone();
                });
            };
