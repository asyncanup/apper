...
App.prototype.init = function (opts) {
    ...
    var log = this.debug.bind(this);
    if (opts && opts.sockets) {
        log("Turning sockets on");
        var socketServer = this.socketServer = http.createServer(this.expressApp),
            socketIO = this.expressApp.socketIO = sockets.listen(this.socketServer);
        ...
        socketIO.sockets.on("connection", function (socket) {
            ...
            socket.on("my other event", function (data) {
                log("Got 'my other event' with data", data);
            });
        });
    }
