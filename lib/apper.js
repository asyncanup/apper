...
function App(path, indent, mountPath) {
    ...
    // then assume it's the root app being initialised
...
App.prototype.debug = function () {
    ...
    console.log.apply(console, [indentText].concat(args));
};
...
App.prototype.init = function () {
    ...
    if (!(envModulePath || middlewarePath || routesModulePath)) {
    ...
    if (fs.existsSync(path + "/public")) {
        this.expressApp.use("/public", express.static(path + "/public"));
    ...
    for (i = 0; i < subappNames.length; i++) {
        ...
        var subapp = new App(path + "/" + name, this.indent, this.mountPath + "/" + name);
...
App.prototype.subappNames = function () {
    ...
    try {
        ...
        console.log("Error reading directory:", path, e.message);
    }
    if (files) {
        files
...
App.prototype.loadModule = function (modulePath) {
    ...
    try {
        ...
        console.log(["Error loading module:", modulePath, e.message].join("\n"));
    }
...
(function () {
    ...
    exports.init = function (mountPath) {
        mountPath = mountPath || ".";
        var routesPath = mountPath + "/routes",
            envPath = mountPath + "/environment",
            middlewaresPath = mountPath + "/middlewares";
        ...
        if (fs.existsSync(routesPath) || fs.existsSync(routesPath + ".js")) {
            try {
                ...
                console.log("\nError in loading module: " + routesPath + "\n" + e.message + "\n");
                return;
            }
    ...
    function getDefaultModules(mountPath) {
        var modulesPresent = [];

        defaultModules.forEach(function (moduleName) {
            var modulePath = mountPath + "/" + moduleName;
            if (fs.existsSync(modulePath) || fs.existsSync(modulePath + ".js")) {
                try {
                    var moduleFunc = require(modulePath);
                    ...
                    console.log("\nError in loading module: " + modulePath + "\n" + e.message + "\n");
                }
            }
        });
    ...
    function loadDefaultModules(modulesPresent, mountPath) {
        modulesPresent.forEach(function (m) {
            console.log("Loading " + mountPath + "/" + m.moduleName);
    ...
    function initPath(fullPath, mountPath) {
        ...
        if (modulesPresent.length > 0) {
            ...
            console.log("Exposed " + mountPath + "/public");
            ...
            console.log("No default modules in " + mountPath + ". Skipping directory.");
        }
