...
App.prototype.init = function () {
    ...
    this.expressApp.mountPath = mountPath;

    log(mountPath || "/");
    this.indent += 1;

    // if environment and middleware exist, load them
    if (envModulePath && this.loadModule(envModulePath)) {
        log("Loaded environment.");
    }
    if (middlewarePath && this.loadModule(middlewarePath)) {
    ...
    if (routesModulePath && this.loadModule(routesModulePath)) {
        log("Loaded routes.");
    }
...
App.prototype.start = function () {
    ...
    // and start listening on the server
    // (which this.socketIO listens to and which includes this.expressApp)
